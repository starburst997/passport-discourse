...
var Provider = null;
var route_callback = null;// = Strategy.route_callback = "/discourse_sso/verify_discourse_sso";
...
function Strategy(options, verify) {
  ...
  if(!route_callback) route_callback = Strategy.route_callback = options.callbackURL;
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  function _verify_discourse_sso(req,res) {
    ...
    self.verify_cb(req,null,null,profile,function(){
      if(ret) {
  ...
  if(s.length > 1 && s[0].length == 0) {
    ...
    var referal_url = "";
    if ( req.headers.referer.startsWith("http://localhost") ) {
      referal_url = "http://localhost" + route_callback + "?redirect=" + encodeURIComponent(req.headers.referer);
    } else if(m && m.length > 1) {
      referal_url = m[1] + "://" + m[2] + route_callback + "?redirect=" + encodeURIComponent(req.headers.referer); 
    } else {
